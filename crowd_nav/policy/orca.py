...
class ORCA(Policy):
    ...
    def predict(self, state):
        ...
        if self.sim is None:
            ...
            self.sim.addAgent((self_state.px, self_state.py), *params, self_state.radius + 0.01 + self.config.robot.robot_human_safety_margin,self_state.v_pref, (self_state.vx, self_state.vy))
            for human_state in state.human_states:
                self.sim.addAgent((human_state.px, human_state.py), *params, human_state.radius + 0.01 + self.config.orca.safety_space,self.max_speed, (human_state.vx, human_state.vy))
        else:
